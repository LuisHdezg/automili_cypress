import{generarNIT, generarTelefono, generarIdentificacion, generarFechaAleatoria, generarFechaAleatoriaa, generarResolucion }from '../funciones.cy';
import faker from "faker";

describe("Entrando a la Pagina MILI Local", () => {
  it("Validando Inicio", () => {
    cy.visit("https://portalrework.miliv2.com/");
    ...
    cy.get('[name="cNature"]').type("PÚBLICO");
    cy.get(".MuiAutocomplete-popper").click();

    const resolucionAleatorio =  generarResolucion();//Numero de Resolucion
    cy.get('[name="cResolutionNum"]').type(resolucionAleatorio, { force: true });

    const fechaAleatoria2 = generarFechaAleatoria();//Fecha Aleatoria
    cy.get('[name="cResolutionDate"]').type(fechaAleatoria2)
