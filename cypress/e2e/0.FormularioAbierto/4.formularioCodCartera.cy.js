...
cy.wait(10000)
    ...
    cy.get('[name="vcDefVal"]').type('10000000') */
   ...
   /*  cy.get('input[name="slider2"]')
    ...
    cy.get('[name="sSellerCode"]').type('3024587964', {force:true})
...
cy.wait(25000)
    ...
    cy.get('[name="payrollStaff.identification"]').type('1143462271');//IDENTIFICACION

    cy.get('[name="payrollStaff.landline"]').click();



    const fechaAleatoria = generarFechanacimmiento();//FECHA DE NACIMIENTO
    cy.wait(1000);
    cy.get('[name="payrollStaff.birthDate"]').type(fechaAleatoria, {force:true});
    ...
    cy.get('[name="payrollBank.typeBranch"]').type('PEREIRA');
    cy.get('.MuiAutocomplete-popper').click();
...
//--------------------------------------------------------------------------------------------------------------------------------
  ...
  //Referencia Particular--------------------------------------------------------------------------------------------------------------------
    ...
    cy.get('[name="payrollReference.cityParticular"]').type('PEREIRA');//CIUDAD
    cy.get('.MuiAutocomplete-popper').click();
...
//---------------------------------------------------------------------------------------------------------------
    ...
    cy.get('[name="payrollRelationships.0.rId"]').type('MADRE');
    cy.get('.MuiAutocomplete-popper').click();

    const nombreBeneficiario = faker.name.findName();
    cy.get('[name="payrollRelationships.0.prName"]').type(nombreBeneficiario);

    const porcentajeBeneficiario1 = porcentajeBeneficiario();
    cy.get('[name="payrollRelationships.0.prPercentage"]').type(porcentajeBeneficiario1);


    cy.wait(3000);

    cy.get('[name="payrollRelationships.1.rId"]').type('TIO(A)');
    cy.get('.MuiAutocomplete-popper').click();

    const nombreBeneficiario1 = faker.name.findName();
    cy.get('[name="payrollRelationships.1.prName"]').type(nombreBeneficiario1);

    const porcentajeBeneficiario2 = porcentajeBeneficiario();
