...
cy.wait(5000)
    ...
    cy.get('[name="payrollStaff.identification"]').type('1143462262');//IDENTIFICACION
    cy.get('body').click();
    cy.wait(3000)
...
cy.get('[name="payrollRelationships.2.rId"]').type('TIO(A)');//PARENTESCO
cy.get('.MuiAutocomplete-popper').click();

const nombreB3 = faker.name.firstName();//NOMBRE DE BENEFICIARIO
const apellidoB3 = faker.name.firstName();//NOMBRE DE BENEFICIARIO
cy.get('[name="payrollRelationships.2.prName"]').type(`${nombreB3} ${apellidoB3}`);

const porcentajeBeneficiario3 = porcentajeBeneficiario();//PORCENTAJE
cy.get('[name="payrollRelationships.2.prPercentage"]').type(porcentajeBeneficiario3);
...
cy.get('[name="payrollRelationships.3.rId"]').type('TIO(A)');//PARENTESCO
cy.get('.MuiAutocomplete-popper').click();

const nombreB4 = faker.name.firstName();//NOMBRE DE BENEFICIARIO
const apellidoB4 = faker.name.firstName();//NOMBRE DE BENEFICIARIO
cy.get('[name="payrollRelationships.3.prName"]').type(`${nombreB4} ${apellidoB4}`);

const porcentajeBeneficiario4 = porcentajeBeneficiario();//PORCENTAJE
cy.get('[name="payrollRelationships.3.prPercentage"]').type(porcentajeBeneficiario4); */
